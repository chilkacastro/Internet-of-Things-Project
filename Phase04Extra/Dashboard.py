...
temperature_Interval =  dcc.Interval(
            id = 'temp-update',
            disabled=False,
            interval = 1*25000,   #lower than 5000 for temperature wouldn't show the temp on the terminal #1800000 equivalent to 30 mins
            n_intervals = 0)
...
def send_led_email_check(value):         # send email and increase the email counter to know there is an email sent
      ...
      if email_counter == 0 and esp_message < light_threshold:
         print("passed here 2")
         sendLedStatusEmail()
         email_counter += 1
